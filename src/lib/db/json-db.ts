...
/** Global BaseEntity fields that db.create() auto-defaults when not provided */
export type DefaultableKeys = 'owner' | 'backup_owner' | 'next_action' | 'next_action_date' | 'stage_or_substatus' | 'exception_flag' | 'file_link'

/**
 * Helper type for entity create functions: makes all DefaultableKeys optional
 * while keeping other fields required.
 */
export type CreateInput<T extends BaseEntity, ExcludeKeys extends string = never> =
  Omit<T, AutoGeneratedKeys | DefaultableKeys | ExcludeKeys> &
  Partial<Pick<T, (DefaultableKeys & keyof T)>> &
  { [K in ExcludeKeys]?: T[K & keyof T] }
...
/** Entity collection names (excludes integration_events which is an array) */
export type EntityCollectionName = Exclude<CollectionName, 'integration_events'>
...
export function create<T extends BaseEntity>(
  collection: EntityCollectionName,
  data: Omit<T, AutoGeneratedKeys | DefaultableKeys> & Partial<Pick<T, DefaultableKeys & keyof T>> & { id?: string },
  actorId: string,
): T {
  const db = readDb()
  ...
  ;(db[collection] as Record<string, BaseEntity & Record<string, unknown>>)[id] = entity as BaseEntity & Record<string, unknown>
