...
const columnHelper = createColumnHelper<Pursuit>()
...
const columns = [
  ...
  columnHelper.accessor('stage', {
    ...
    cell: (info) => (
      <StatusBadge
        state={info.getValue()}
        label={PURSUIT_STAGE_LABELS[info.getValue()]}
      />
    ),
  }),
  columnHelper.accessor('build_type', {
    ...
    cell: (info) => {
      ...
      return val ? PURSUIT_BUILD_TYPE_LABELS[val] : '—'
    },
  }),
  columnHelper.accessor('approx_sqft', {
    header: 'Approx SF',
    cell: (info) => {
      const val = info.getValue()
...
interface PursuitTableProps {
  pursuits: Pursuit[]
}

export function PursuitTable({ pursuits }: PursuitTableProps) {
  const [sorting, setSorting] = useState<SortingState>([])
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
  const [globalFilter, setGlobalFilter] = useState('')

  const data = useMemo(() => pursuits, [pursuits])
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground">
          {table.getFilteredRowModel().rows.length} pursuit{table.getFilteredRowModel().rows.length !== 1 ? 's' : ''}
        </span>
      </div>
