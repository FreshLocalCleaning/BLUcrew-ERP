...
import * as jsonDb from '@/lib/db/json-db'
...
export async function compressedPlanningOverrideAction(
  ...
  // exception_flag is a BaseEntity field, so use jsonDb.update directly
  const updated = jsonDb.update<Mobilization>(
    'mobilizations',
    parsed.data.mobilization_id,
    { compressed_planning: true, exception_flag: true } as Partial<Omit<Mobilization, jsonDb.AutoGeneratedKeys>>,
    actor.id,
